...
SPEC = importlib.util.spec_from_file_location("test_public_procurement_research_adapter", REPO_ROOT / "kgov_runtime" / "capabilities" / "public_procurement_research.py")
...
class AdapterTest(unittest.TestCase):
    def test_fixture_mode_is_deterministic(self) -> None:
        payload = json.loads((REPO_ROOT / "tests" / "fixtures" / "capabilities" / "public-procurement-research.json").read_text(encoding="utf-8"))
        from kgov_runtime.http import normalize_records
        result = normalize_records(payload)
        self.assertEqual(1, result["count"])
        self.assertEqual("public-procurement-research", result["records"][0]["capability"])
