...
SPEC = importlib.util.spec_from_file_location("test_kosis_official_statistics_adapter", REPO_ROOT / "kgov_runtime" / "capabilities" / "kosis_official_statistics.py")
...
class AdapterTest(unittest.TestCase):
    def test_fixture_mode_is_deterministic(self) -> None:
        payload = json.loads((REPO_ROOT / "tests" / "fixtures" / "capabilities" / "kosis-official-statistics.json").read_text(encoding="utf-8"))
        from kgov_runtime.http import normalize_records
        result = normalize_records(payload)
        self.assertEqual(1, result["count"])
        self.assertEqual("kosis-official-statistics", result["records"][0]["capability"])
    ...
    def test_live_query_path_is_mocked_and_read_only(self) -> None:
        ...
        self.assertIn("apiKey=fixture-secret", opened[0])
