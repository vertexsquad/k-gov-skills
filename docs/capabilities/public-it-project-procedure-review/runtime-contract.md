# Runtime contract — `public-it-project-procedure-review`
...
- 출력: count/status/check identifiers만 포함하는 `procedure-review-draft-only` 초안 admission.
- side effect: 없음. 네트워크 조회, 원본 변경, 제출, 게시, 발송을 수행하지 않습니다.
- 증거 상태: `fixture-verified`; live smoke는 catalog manifest 값을 따릅니다.
- 금지 판단: `automatic-procurement-start, automatic-acceptance, institution-rule-assumption`.
