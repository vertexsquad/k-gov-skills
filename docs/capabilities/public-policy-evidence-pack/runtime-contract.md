...
- `domains/<domain>/skills/<slug>/SKILL.md`는 질문 분해와 기존 read-only capability 조합 절차를 안내합니다.
- `python3 -m kgov_runtime.capabilities.public_policy_evidence_pack`는 caller가 준비한 normalized evidence ledger만 local에서 검증합니다.
