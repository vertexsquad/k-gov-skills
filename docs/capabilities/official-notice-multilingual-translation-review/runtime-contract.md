# Runtime contract — `official-notice-multilingual-translation-review`
...
- 출력: count/status/check identifiers만 포함하는 `translation-review-draft-only` 초안 admission.
- side effect: 없음. 네트워크 조회, 원본 변경, 제출, 게시, 발송을 수행하지 않습니다.
- 증거 상태: `fixture-verified`; live smoke는 catalog manifest 값을 따릅니다.
- 금지 판단: `automatic-publication, legal-meaning-certification, medical-meaning-certification`.
- live gap: 언어별 전문 감수와 기관별 공식 번역 workflow는 검증하지 않았습니다.
