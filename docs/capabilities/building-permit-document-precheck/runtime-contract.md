# Runtime contract — `building-permit-document-precheck`
...
- 출력: count/status/check identifiers만 포함하는 `permit-precheck-draft-only` 초안 admission.
- side effect: 없음. 네트워크 조회, 원본 변경, 제출, 게시, 발송을 수행하지 않습니다.
- 증거 상태: `fixture-verified`; live smoke는 catalog manifest 값을 따릅니다.
- 금지 판단: `permit-eligibility-decision, drawing-compliance-approval, automatic-application-submission`.
- live blocker: 세움터 진입점의 자기 redirect가 관찰되어 실제 민원 기능을 검증하지 않았습니다.
